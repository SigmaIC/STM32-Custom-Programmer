...
namespace STM32CustomProg
{
    ...
    public class STLinkAdapter
    {
        ...
        private const string CmdListDevices = "-l st-link";
        private const string CmdConnectToTarget = "-c port=SWD";
        private const string CmdProgramFromFile = "-w";
        private const string CmdFlashErase = "-e all";
        private const string CmdVerifyProgramming = "-v";
        private const string CmdRunFirmware = "-s";
        private const string CmdNoPrompt = "-NoPrompt";
        private const string STLinkCLIDefaultPath = @"C:\Program Files (x86)\STMicroelectronics\STM32Cube\STM32CubeProgrammer\bin\STM32_Programmer_CLI.exe";
        ...
        public STLinkReturnCodes ConnectToTarget(string a_TargetProcessor, out string a_ResultOut)
        {
            ...
            if (l_STLinkReturnCode == STLinkReturnCodes.Success)
            {
                ...
                {
                    a_ResultOut += "\nTarget processor found: " + a_TargetProcessor;
        ...
        //----------------------------------- 
        public STLinkReturnCodes ProgramTarget(string a_BinFilePath, UInt32 a_FlashAddress, string a_FlashSN, out string a_ResultOut)
        {
            string CmdFlasherSN = "sn=" + a_FlashSN;
            STLinkReturnCodes l_STLinkReturnCode = STLinkReturnCodes.Success;
            ...
            if (l_STLinkReturnCode == STLinkReturnCodes.Success)
            {
                string l_CommandArguments = string.Format("{0} {1} {2} {3} {4} 0x{5:X8} {6} {7}", CmdConnectToTarget, CmdFlasherSN, CmdFlashErase, CmdProgramFromFile, a_BinFilePath, a_FlashAddress, CmdVerifyProgramming, CmdRunFirmware);

                l_STLinkReturnCode = ExecuteCommandSync(STLinkCLIAppPath, l_CommandArguments, out a_ResultOut);
                if (l_STLinkReturnCode == STLinkReturnCodes.Success)
                {
                    if (a_ResultOut.Contains("Download verified successfully"))
                    {
